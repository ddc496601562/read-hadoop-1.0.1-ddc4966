...
public class JobInProgress {
  ...
  /**
   * 
   * @param splits   map任务的所有输入split ，和TaskInProgress maps[]对应，每个maps[i]处理splits[i]的数据  
   * @param maxLevel
   * @return
  ...
  private Map<Node, List<TaskInProgress>> createCache(
    ...
    for (int i = 0; i < splits.length; i++) {
      ...
      for(String host: splitLocations) {
        ...
        //maxLevel=2 ，所以就是机器-机架  ，认为机架也算是local化的，其它认为都是一样的网络距离
        ...
        for (int j = 0; j < maxLevel; j++) {
          ...
          //i是大循环中的，每次大循环maps[i]都是一个值,所以若是已经添加过了，也是刚才刚添加进去的，很蹩脚的一段代码
   ...
   * 为一个空闲的tasktracker寻找一个最优执行的map
   */
  public synchronized Task obtainNewMapTask(TaskTrackerStatus tts, 
                                            int clusterSize, 
                                            int numUniqueHosts
                                           ) throws IOException {
    //状态是running才接受调度，其它都不接受，为啥为调度到非running的job呢 ？
