...
public class JobSplitWriter {
  ...
  /**
   * 将split的具体信息写入该job在hdfs中的submitDir中，写入的信息有：
   * 1、split信息，详见{@link #writeNewSplits(Configuration, InputSplit[], FSDataOutputStream)}
   * 2、每个split的offset、length信息、所在的机器host的信息
   * @param jobSubmitDir
   * @param conf
   * @param fs
   * @param splits
  ...
  throws IOException, InterruptedException {
	/**
	 * 将split的信息写入submitJobdir/job.split文件中，每个split写入的信息是：
	 * split的类名称(String)、split的序列化信息（使用的是serializer序列化，可以扩展）
	 * 返回的信息是每个Split的具体信息：offset、length、文件所在的机器host
	 */
