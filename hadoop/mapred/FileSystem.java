...
public abstract class FileSystem extends Configured implements Closeable {
  ...
  public static FileSystem parseArgs(String argv[], int i, Configuration conf) throws IOException {
    ...
    */
    ...
    if ("-dfs".equals(cmd)) {
      ...
      fs = (FileSystem)ReflectionUtils.newInstance(ReflectionUtils.getConstructor("org.apache.hadoop.hdfs.DistributedFileSystem", InetSocketAddress.class, Configuration.class), addr, conf);
  ...
  /**
   * same to createNewFile but add permission.
   * add by baidu-sos-lxn
   */
  public boolean createNewFile(Path f, FsPermission permission) throws IOException {
    if (exists(f)) {
      return false;
    } else {
      this.create(f, permission, false, getConf().getInt("io.file.buffer.size", 4096), getDefaultReplication(), getDefaultBlockSize(), null).close();
      return true;
    }
  }
  ...
  public IFileStatusWithLoc[] listStatusWithLoc(Path f) throws IOException {
    throw new IOException("not support list status with location");
  }
  ...
  private void listStatus(ArrayList<FileStatus> results, Path f,
    ...
    boolean withLoc = "true".equals(
        System.getProperty("fs.list.withloc", "false"));
    
    if(withLoc){
      IFileStatusWithLoc listing[] = listStatusWithLoc(f);
      if (listing != null) {
        for (int i = 0; i < listing.length; i++) {
          if (filter.accept(listing[i].getPath())) {
            results.add((FileStatus)listing[i]);
          }
        }
      }
    }else{
  ...
  public FileStatus[] globStatus(Path pathPattern, PathFilter filter)
      throws IOException {
    if ("false".equals(System.getProperty("glob"))) {
  ...
  public Path getHomeDirectory() {
	  ...
	  String[] ugi = this.getConf().getStrings("hadoop.job.ugi");
	  if ((ugi != null) && (ugi.length >= 1)){
		  String username = ugi[0].trim();
...
//      .makeQualified(this);
  ...
  public void setReadCorrupt(boolean readCorrupt){
  }
  
  public void setVerifyChecksum(boolean verify){
  }
