...
public class DFSClient implements FSConstants, java.io.Closeable {
  ...
  public class DFSInputStream extends FSInputStream {
    ...
    synchronized void openInfo() throws IOException {
      //通过调用namenode的RPC端口，获取该文件所有的块，以及每个块所在的datanode节点
     ...
     * 缓存的读法，DFSInputStream中保存着一个和datanode通信的socket，然后每次读的时候从pos开始，
     * DFSInputStream中始终维护者当前的pos和当前的块的socket 。
     * 在读取一个文件时候，这个方法和绝对position读取(#link int read(long position, byte[] buffer, int offset, int length)#)的差别在于：
     * 这个在读取一个block时候，即使没有读完所有的block数据，也不会关闭socket 
     * 但是第二个方法会，每次读取完毕会关闭socket，及时只读到一个快的中间block_pos，下次若是接着block_pos读取需要重新代开socket
     */
    @Override
    ...
    //根据block的信息，选择第一个没有dead的datanode节点读取块数据 。
    private DNAddrPair chooseDataNode(LocatedBlock block)
      throws IOException {
      while (true) {
        DatanodeInfo[] nodes = block.getLocations();
        try {
        //选择正常的（非 dead datanode）节点读取数据
    ...
    //在块block上读取从start开始到end结束的数据，总共(end-start+1)字节到buf中，buf从offset开始写入数据
    private void fetchBlockByteRange(LocatedBlock block, long start,
      ...
      while (true) {
        ...
        try {
          ...
          // first try reading the block locally.  如 果这个块在本地的话
          if (shouldTryShortCircuitRead(targetAddr)) {
            try {
    ...
    public int read(long position, byte[] buffer, int offset, int length)
      ...
      for (LocatedBlock blk : blockRange) {
    	  //targetStart  在这个块上的开始位置
        long targetStart = position - blk.getStartOffset();
        //在这块上需要读的字节数：要么是读的数目够了，要么是把这个块的数据读完
