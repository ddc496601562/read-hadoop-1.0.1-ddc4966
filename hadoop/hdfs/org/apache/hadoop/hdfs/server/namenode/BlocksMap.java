...
class BlocksMap {
   ...
   * 这个类存储了一个block的具体信息，所在的datanode、前一个备份快，后一个备份快
   * 这个类存储了N个备份block的信息
   */
  static class BlockInfo extends Block implements LightWeightGSet.LinkedElement {
     ...
     * 数据是：DatanodeDescriptor(第一个备份 块所在的datanode),BlockInfo(第一个备份 前一个块),BlockInfo(第一个备份 后一个块),
     *       DatanodeDescriptor(第二个备份 块所在的datanode),BlockInfo(第二个备份 前一个块),BlockInfo(第二个备份 后一个块),
     *       ..........
     *       第N个备份所在的datanode ，block的前一个快，block的后一个块
     */
