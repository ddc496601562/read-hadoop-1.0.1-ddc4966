...
abstract class INode implements Comparable<byte[]> {
  ...
  private static enum PermissionStatusFormat {
	//一个64位的long型bits，其最低16位表示权限，然后接着25位表示group，然后接着23位表示user
    MODE(0, 16),
    ...
    PermissionStatusFormat(int offset, int length) {
      ...
      //这句话其实是构造了一个64位的bit数组，其从倒数第OFFSET个位置开始LENGTH的元素全是1 
    }
    //将record的bits的倒数第OFFSET个位置开始LENGTH的元素保留，然后再移到小端
    //其实就是取出permission中相对应的group 或者 user 或者 文件权限 的 相应的位的值
    //record位相应的permission
    long retrieve(long record) {
      return (record & MASK) >>> OFFSET;
    }
    //record仍然为permission，该方法的作用是将bits表示的权限设置到group 或者 user 或者 文件权限相应的位置中
    //bug ？若是bit不按照正常的相应的group 或者 user 或者 文件权限格式，即通过bits << OFFSET后的高位仍有1，这回影响以他权限
