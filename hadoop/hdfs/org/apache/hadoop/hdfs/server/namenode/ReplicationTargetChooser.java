...
class ReplicationTargetChooser {
   ...
   * @param writer: the writer's machine, null if not in the cluster.
   * @param choosenNodes: datanodes that have been choosen as targets.  曾经作为这个块的存储的datanode
   * @param excludedNodes: datanodesthat should not be considered targets.
   * @param blocksize: size of the data to be written.
  ...
  DatanodeDescriptor[] chooseTarget(int numOfReplicas,
    ...
    //待复制的份数+已经用过的nodes数(正常存储的、已经废掉的、block在其上面损坏的....) 不能超过集群总的nodes数目
  ...
  private DatanodeDescriptor chooseTarget(int numOfReplicas,
    ...
    //其实不合理，results中包含的可能是已经不能用了的DatanodeDescriptor  ？？？？ 8888888
